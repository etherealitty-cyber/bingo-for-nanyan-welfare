...
create or replace function public.bingo_admin_answers(
  p_token text,
...
begin
  ...
  if p_answers is null
    ...
    or jsonb_object_length(p_answers) = 0
    or jsonb_object_length(p_answers) > 25
  then
  ...
  if v_updated_count <> jsonb_object_length(p_answers) then
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'filled_count', (
      select coalesce(sum(
        (coalesce(entry.value ->> 'yesParticipantId', '') <> '')::integer
        + (coalesce(entry.value ->> 'noParticipantId', '') <> '')::integer
      ), 0)
      from jsonb_each(coalesce(d.entries, '{}'::jsonb)) as entry
    ),
    ...
    case
      ...
      when d.participant_id is not null then 1
      else 2
    end,
...
revoke all on function public.bingo_admin_answers(text, text) from public;
...
grant execute on function public.bingo_admin_answers(text, text) to anon;
